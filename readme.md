...
Проект разрабатывается по этапам. Текущая ветка `feature-05-persistent-photo-index` добавляет локальный индекс обработанных фотографий, чтобы прогресс EXIF-сканирования не терялся между запусками.
